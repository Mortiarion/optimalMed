...
const links = [
    { title: "Дерматолог", url: "dermatologist.html" },
    { title: "Иммунолог", url: "imunologist.html" },
    { title: "Кардиолог", url: "cardiologist.html" },
    { title: "Кардиохирург", url: "cardiac-surgeon.html" },
    { title: "Нарколог", url: "narcologist.html" },
    { title: "Невролог", url: "neurologist.html" },
    { title: "Невролог", url: "neurosurgeon.html" },
    { title: "Онколог", url: "oncologist.html" },
    { title: "Уролог", url: "urolog.html" },
];
...
function generateLinksList(links) {
    return links
        .map(
            (link) => `
            <li>
                <button onclick="openWindowWithUrl('${link.url}');">${link.title}</button>
            </li>`
        )
        .join("");
}

const renderLinksArticleCol2 = document.getElementById(
    "render-links-article-col-2"
);
renderLinksArticleCol2.innerHTML = generateLinksList(links);
...
function openWindowWithUrl(url) {
    ...
    if (newWindow) {
        newWindow.addEventListener("DOMContentLoaded", function () {
            renderArticleHtmlPages(newWindow, url);
        });
    }
}

const renderArticleHtmlPages = (newWindow, fileName) => {
    ...
    const data = {
        headTitle: "Робота урологом в Польше",
        content: "Польща уролог",
    };
